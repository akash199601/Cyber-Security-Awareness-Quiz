...
class CandidateForm(forms.Form):
    employee_id = forms.CharField(label="Employee ID", max_length=20, required=True,  widget=forms.TextInput(attrs={'class': 'form-control','id':'emp_id'}))
    ...
    def clean_employee_id(self):
        ...
        if employee_id != 'admin':
            if not employee_id.isdigit():
                raise forms.ValidationError("Employee ID should only contain positive numbers.")
